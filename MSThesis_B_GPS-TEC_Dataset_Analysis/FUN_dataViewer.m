...
dayNite_sunrise = zeros(dayNite_Grid_size,dayNite_Grid_size,size(dateRange_full,1),'single'); %hr UT, prep sunrise time for each day in the lat/long grid
dayNite_sunset = zeros(dayNite_Grid_size,dayNite_Grid_size,size(dateRange_full,1),'single'); %hr UT, prep sunset time for each day in the lat/long grid
[dayNite_Grid_Long,dayNite_Grid_Lat] = meshgrid(linspace(min(plotLongRange),max(plotLongRange),dayNite_Grid_size),linspace(min(plotLatRange),max(plotLatRange),dayNite_Grid_size)); %degc, make two matrixes that have all the corresponding points in a lat/long grid
dayNite_long_corrected = 4*(dayNite_Grid_Long); %calc corrected longitude, for sunrise/sunset time

for( i = 1:dateRange_numDays )

    dayNite_B = 360*(dateRange_full(i,2) - 81)/365; %some sort of angle based on days and stuff
    dayNite_EoT_corrected = 9.87*sind(2*dayNite_B) - 7.53*cosd(dayNite_B) - 1.5*sind(dayNite_B); %eq for Time Correction
    dayNite_solar_corrected = dayNite_long_corrected + repmat(dayNite_EoT_corrected,dayNite_Grid_size,dayNite_Grid_size); %min, solar time correction - for noon

    dayNite_solar_declination = asind(sind(23.45)*sind(360*(dateRange_full(i,2) - 81)/365)); %deg, solar declination

    dayNite_temp = -tand(dayNite_Grid_Lat).*tand(dayNite_solar_declination); %calc some mid step
%     dayNite_temp( dayNite_temp >= 1 ) = dayNite_temp( dayNite_temp >= 1 ) - -2*(1 - dayNite_temp( dayNite_temp >= 1 )); %attempt a flip
%     dayNite_temp( dayNite_temp <= -1) = dayNite_temp( dayNite_temp <= -1) + -2*(1 + dayNite_temp( dayNite_temp <= -1) ); %attempt a flip
    k = (dayNite_temp(:,1) <= -1) | (dayNite_temp(:,1) >= 1); %prep to replace these
%     kF = find(k == 1,1,'first');
    
    dayNite_sunrise(:,:,i) = 12 - real(acosd(dayNite_temp))/15 - dayNite_solar_corrected/60; %hr UT, sunrise time
%     for(j = 1:dayNite_Grid_size) %interpolation didn't work - too precipitous
%         dayNite_sunrise(k,j,i) = interp1(1:1:(kF-1),dayNite_sunrise(~k,j,i),kF:1:dayNite_Grid_size,'spline','extrap'); %interp for each long set (calc craps out at specific lat)
%     end
    dayNite_sunset(:,:,i) = 12 + real(acosd(dayNite_temp))/15 - dayNite_solar_corrected/60; %hr UT, sunrise time
    %Accurate to like 10 minutes or whatever (breaks at high latitudes...)
    
    dayNite_sunrise(k,:,i) = NaN; %remove data that we can't calc with this alg
    dayNite_sunset(k,:,i) = NaN; %remove data that we can't calc with this alg
...
dayNite_sunrise = reshape(dayNite_sunrise,dayNite_Grid_size,dayNite_Grid_size*dateRange_numDays); %reshape into one big thing since each day's 6 AM sunrise or whatever is now per-day (+/-24 etc)
dayNite_sunset = reshape(dayNite_sunset,dayNite_Grid_size,dayNite_Grid_size*dateRange_numDays); %reshape into one big thing since each day's 6 AM sunrise or whatever is now per-day (+/-24 etc)
dayNite_Grid_Lat = repmat(dayNite_Grid_Lat,1,dateRange_numDays); %copy this to match above 1:1
dayNite_Grid_Long = repmat(dayNite_Grid_Long,1,dateRange_numDays); %copy this to match above 1:1
...
% if( gif_plotProjection == 0 ) %causes weird clipping on edges where some NaNs get kicked off that were needed
%     jk = find(coastLines_lat < min(plotLatRange) ); %find less than min, remove
%     coastLines_lat(jk) = []; %remove
%     coastLines_long(jk) = []; %remove
%     jk = find(coastLines_lat > max(plotLatRange) ); %find more than max, remove
...
if( gif_plotProjection == 1 && gif_Type == 4 ) %revert plot projection if gif_Type can't do plot projection
    gif_plotProjection = 0; %that form of mapping is not supported atm sorry - seems map axes can only have 1 color map between them (ruins multi-color map)
    %and I can't make that shape like no
    warning(['MOVIE CREATION ISSUE: Plot Projection (gif_plotProjection = 1) set but movie requires 2 or more color maps, which is not supported in projected maps atm thx MatlaB (for ref: you set gif_Type = ',num2str(gif_Type),') not my fault']);
end

fig1 = figure('Color','w','units','normalized','outerposition',[0 0 1 1]); %maximizes figure window, makes it white
if( gif_plotProjection == 0 )
    %This option plots without mapping projections
    
    fig1Axes = axes('Parent', fig1); %gets a reference to the axes for later - NOT A MAP AXIS axes set commands
    
    xlabel(fig1Axes,'Longitude (degc)'); %old: ,'fontweight','bold','FontSize',12 for all
    ylabel(fig1Axes,'Latitude (degc)'); %sped up significantly when merged above
    title(fig1Axes,'Prep Title'); %fire up the title
    set(gca, 'fontweight',font_Weight, 'FontSize',font_Size);
    hold on;
    
    if( gif_ContinentFill == 1 ) %This option fills in the continents/water
        hMap = geoshow(fig1Axes,coastLines_lat,coastLines_long,'DisplayType','polygon','FaceColor', gif_ContientColor);
        set(fig1Axes,'DataAspectRatioMode','auto'); %was getting set to manual causing [1,1,1] ratio so it was tiny *UGH*
        xlim(fig1Axes,[min(plotLongRange), max(plotLongRange)]); %force axis to be this
        ylim(fig1Axes,[min(plotLatRange), max(plotLatRange)]); %force axis to be this
        set(fig1Axes,'Color',gif_ContinentWaterColor); %Colors the water blue
        
    else %Otherwise it is white
        %Plot lines of contients
        hMap = geoshow(fig1Axes,coastLines_lat,coastLines_long,'Color','k'); %add continental outlines in black  
        xlim(fig1Axes,[min(plotLongRange), max(plotLongRange)]); %force axis to be this
        ylim(fig1Axes,[min(plotLatRange), max(plotLatRange)]); %force axis to be this
        box(fig1Axes,'on'); %outlines on all sides forced
    end
    
else
%     if(gif_Type == 4 ) %mapping axes cannot be hidden - so this is made 1st so it's behind the real one ANNOYING AM I RIGHT
%         ax1 = worldmap([min(plotLatRange) , max(plotLatRange)],[min(plotLongRange) , max(plotLongRange)]); %create independent axes for the two plots (TEC and AMPERE)
%         hold on;
%         ax2 = worldmap([min(plotLatRange) , max(plotLatRange)],[min(plotLongRange) , max(plotLongRange)]); %create independent axes for the two plots (TEC and AMPERE)
%         %They're geographic so they match! Genius! BUT THEY NEED TO BE HERE UGH
%         hold on;
%     end
    
    %Otherwise use the default map projection method
    fig1Axes = worldmap([min(plotLatRange) , max(plotLatRange)],[min(plotLongRange) , max(plotLongRange)]); %tries to standardize the maps - THIS IS A MAP AXIS axesm setm 
    
    %THESE DON'T SHOW UP IN PROJECTED MAPS
    xlabel('Longitude (degc)'); %old: ,'fontweight','bold','FontSize',12 for all
    ylabel('Latitude (degc)'); %sped up significantly when merged above
    %these do
    title('Prep Title'); %fire up the title
    set(gca, 'fontweight',font_Weight, 'FontSize',font_Size);
    hold on;
    
    if( gif_plotProjection_Mercator == 1 )
        setm(gca,'mapprojection','mercator'); %applies the Mercator projection
    end %otherwise let it just do whatever it feels (may add more options if needed)
    
    if( gif_ContinentFill == 1 )
        %This option fills in the continents/water - worldmap axes prep limits axes already
        hMap = geoshow(fig1Axes,coastLines_lat,coastLines_long,'DisplayType','polygon','FaceColor', gif_ContientColor);
        setm(gca,'FFaceColor', gif_ContinentWaterColor); %FFaceColor sets ocean color (orig color [.5,.7,.9]) powder blue: [176/255,224/255,230/255]
        
    else %Otherwise it is white
        %Plot lines of contients - worldmap axes prep limits axes already
        hMap = geoshow(fig1Axes,coastLines_lat,coastLines_long,'Color','k'); %add continental outlines in black  
    end
    
    if( gif_plotProjection_Mercator == 1 ) %Mercator does better with tight axes expanding
        set(fig1Axes,'DataAspectRatioMode','auto'); %was getting set to manual causing [1,1,1] ratio so it was tiny *UGH*
        InSet = get(fig1Axes, 'TightInset'); %super call to force it to fill the area
        set(fig1Axes, 'Position', [InSet(1:2), 1-InSet(1)-InSet(3), 1-InSet(2)-InSet(4)]); %force the filling
    else %others do better with auto scaling
        set(fig1Axes,'DataAspectRatioMode','auto'); %was getting set to manual causing [1,1,1] ratio so it was tiny *UGH*
    end
    
    %Convert to X/Y coords as needed
    mstruct = gcm; %call up the current map stuff for good calcs
    [pplong_Circ,pplat_Circ] = mfwdtran(mstruct,pplat,pplong); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    [plotLongRange_Circ,plotLatRange_Circ] = mfwdtran(mstruct,plotLatRange,plotLongRange); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    [longMillstone_Circ,latMillstone_Circ] = mfwdtran(mstruct,latMillstone,longMillstone); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    [longMillstoneMISA_Circ,latMillstoneMISA_Circ] = mfwdtran(mstruct,latMillstoneMISA,longMillstoneMISA); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    if( FLG_AMPERE_DATA == 1 )
        [AMPERE_long_Circ,AMPERE_lat_Circ] = mfwdtran(mstruct,AMPERE_lat,AMPERE_long); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    end
end
defaultPosition = get(fig1Axes,'Position'); %get orig figure position 
defaultAspectRatio = get(fig1Axes,'PlotBoxAspectRatio'); %get orig figure aspect ratio
...
% if( defaultAspectRatio(1) < defaultAspectRatio(2) )%defaultAspectRatio(1) X is less than defaultAspectRatio(2) Y - which means that to square it Y grid needs to go up while X goes down (to maintain total grid number)
%     gif_Grid_AdjustmentRatio = defaultAspectRatio(1)/defaultAspectRatio(2); %get the adjustment ratio needed
%     gif_Grid_Lat_Spaces = round(gif_Grid_Lat_Spaces*gif_Grid_AdjustmentRatio); %apply the ratio by boosting Y
%     gif_Grid_Long_Spaces = round(gif_Grid_Long_Spaces/gif_Grid_AdjustmentRatio); %apply the ratio by reducing X
...
%         gif_Grid_TotalGrids_LatNearness = abs(abs(gif_Grid_TotalGrids_Delta) - gif_Grid_Lat_Spaces); %find how close lat can fix the delta
%         gif_Grid_TotalGrids_LongNearness = abs(abs(gif_Grid_TotalGrids_Delta) - gif_Grid_Long_Spaces); %find how close long can fix the delta
...
%         
%     end
% else %otherwise defaultAspectRatio(2) Y is less than defaultAspectRatio(1) X - which means that to square it X grid needs to go up while Y goes down (to maintain total grid number)
%     gif_Grid_AdjustmentRatio = defaultAspectRatio(1)/defaultAspectRatio(2); %get the adjustment ratio needed
%     gif_Grid_Lat_Spaces = round(gif_Grid_Lat_Spaces/gif_Grid_AdjustmentRatio); %apply the ratio by reducing Y
%     gif_Grid_Long_Spaces = round(gif_Grid_Long_Spaces*gif_Grid_AdjustmentRatio); %apply the ratio by boosting X
%     gif_Grid_TotalGrids_Adj = gif_Grid_Lat_Spaces*gif_Grid_Long_Spaces; %get new adjusted grid points
%     if( gif_Grid_TotalGrids ~= gif_Grid_TotalGrids_Adj ) %if adjusted sizes doesn't have the same number as the orig, fix it
%         gif_Grid_TotalGrids_Delta = gif_Grid_TotalGrids_Adj - gif_Grid_TotalGrids; %get delta between two numbers
%         gif_Grid_TotalGrids_LatNearness = abs(abs(gif_Grid_TotalGrids_Delta) - gif_Grid_Lat_Spaces); %find how close lat can fix the delta
...
if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
    gif_Grid_Lat = linspace(min(plotLatRange),max(plotLatRange),gif_Grid_Lat_Spaces+1); %degc, create lat points (+1 lets us use delta to edge wanted range - yields correct # of spaces)
    gif_Grid_Long = linspace(min(plotLongRange),max(plotLongRange),gif_Grid_Long_Spaces+1); %degc, create long points (+1 lets us use delta to edge wanted range - yields correct # of spaces)
    gif_Grid_Lat_Delta = abs(gif_Grid_Lat(2) - gif_Grid_Lat(1)); %degc, lat delta
    gif_Grid_Long_Delta = abs(gif_Grid_Long(2) - gif_Grid_Long(1)); %degc, long delta
    
%     gif_Grid_Lat_Plot = gif_Grid_Lat(1:end-1); %degc, plotting range for plots n such
%     gif_Grid_Long_Plot = gif_Grid_Long(1:end-1); %degc, plotting range for plots n s
else %projection requires converting to X/Y coords and then using them if plotting normally (mapping plot functions can use degc still - like geoshow)
    gif_Grid_Lat = linspace(min(plotLatRange),max(plotLatRange),gif_Grid_Lat_Spaces+1); %degc, create lat points (+1 lets us use delta to edge wanted range - yields correct # of spaces)
    gif_Grid_Long = linspace(min(plotLongRange),max(plotLongRange),gif_Grid_Long_Spaces+1); %degc, create long points (+1 lets us use delta to edge wanted range - yields correct # of spaces)
    [gif_Grid_Long_Circ,gif_Grid_Lat_Circ] = meshgrid(gif_Grid_Long,gif_Grid_Lat); %make two matrixes that have all the corresponding points
    gif_Grid_Lat_Circ = reshape(gif_Grid_Lat_Circ,(gif_Grid_Lat_Spaces+1)*(gif_Grid_Long_Spaces+1),1); %reshape into a linear array for the mfwdtran fun
    gif_Grid_Long_Circ = reshape(gif_Grid_Long_Circ,(gif_Grid_Lat_Spaces+1)*(gif_Grid_Long_Spaces+1),1); %reshape into a linear array for the mfwdtran fun
    [gif_Grid_Long_Circ,gif_Grid_Lat_Circ] = mfwdtran(mstruct,gif_Grid_Lat_Circ,gif_Grid_Long_Circ); %convert lat/long in degc to X/Y in m [uses arclength = radius*angle formula with a 0 somewhere] 
    %Basically the transforms can have 30 deg lat is not the same Y value
    gif_Grid_Lat_Circ = reshape(gif_Grid_Lat_Circ,(gif_Grid_Lat_Spaces+1),(gif_Grid_Long_Spaces+1)); %reshape into a linear array for the mfwdtran fun
    gif_Grid_Long_Circ = reshape(gif_Grid_Long_Circ,(gif_Grid_Lat_Spaces+1),(gif_Grid_Long_Spaces+1)); %reshape into a linear array for the mfwdtran fun
    
%     gif_Grid_Lat_Plot = gif_Grid_Lat_Circ(1:end-1,1:end-1); %degc, plotting range for plots n such
%     gif_Grid_Long_Plot = gif_Grid_Long_Circ(1:end-1,1:end-1); %degc, plotting range for plots n such
end

%----------------------PREP FOR MOVIE MAKING-------------------------------
origDirectory = pwd; %record original directory
gif_Save_Locale = pwd; %temp overwrite (remove if no want)
cd(gif_Save_Locale); %move to desired location

%need to do same frame for a while to make it good
gif_DesiredFPS = round(1/gif_DesiredFrameTime); %frames/sec, desired frames per sec from the frame time (how long a frame is on the screen)

if( gif_MP4 == 1 ) %supports choosing video or GIF
    v = VideoWriter([gif_Name,'.mp4'],'MPEG-4');
    if( gif_Type == 0 || gif_Type == 1 || gif_Type == 2 || gif_Type == 3 || gif_Type == 5) %supports doubling frame rate for the movies not using time-averaging (they do their own thing)
        if( ( length(timeUnique)/30 > gif_DesiredMaxRunTime ) && ( gif_FLGdisableFPSShift == 0 ) ) %try to keep run time down
            v.FrameRate = 60; %set the FPS to 60
        end
    end
else
    FLG_firstPic = 1; %flag, 1 to start off
    gif_Name = [gif_Name,'.gif']; %gif name
end
...
if(gif_Type == 0) %moving gif implementation (scatter points)
    ...
    %------------------------Initialize Video------------------------------
    if( gif_MP4 == 1 ) %open here to support last-minute changes to FPS, etc
        open(v); %open video
    end
    ...
    for( i = 1:length(timeUnique) )
        ...
        %----------------Corral the data to the right place----------------
        k = find( time == timeUnique(i)); %gets during a time period
        
        %----------------------------Tack on Title-------------------------
        string_Title = ['TEC Global Plot, Time = ',num2str(round((timeUnique(i) - dateRange_zeroHr(2))*24,2),'%10.2f'),' hr, 0 UT on ',dateRange_zeroHr_YrMonDay_string]; %create title string
        if( FLG_geomagneticCoords == 1)
            string_Title = [string_Title,' [Mag Coord Aligned]']; %tack on mag coordinate warning
        end
        title(fig1Axes,string_Title);

        %-------------------Starting the Plotting--------------------------
        if( i == 1 ) %first run preps axes, color bars, etc.
            if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
                %Do the TEC plotting
                h = scatter(fig1Axes,pplong(k),pplat(k),gif_Scatter_Point_Size,sTEC(k),'filled');
                caxis(fig1Axes,[-gif_sTEC_cap, gif_sTEC_cap]);
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                colormap(fig1Axes,'jet') %heat map
                hBar = colorbar(fig1Axes); %shows the color bar 
                ylabel(hBar, 'delta-sTEC (TECU)','fontweight',font_Weight, 'FontSize',font_Size); %labels color bar
                set(fig1Axes,'xtick',min(plotLongRange):plotLongRange_autoTick:max(plotLongRange)); %set standardized limit
                set(fig1Axes,'ytick',min(plotLatRange):plotLatRange_autoTick:max(plotLatRange)); %set standardized limit

                %Plot lines of contients
                hOverlay = geoshow(fig1Axes,coastLines_lat,coastLines_long,'Color','k'); %add continental outlines in black  

                %Now drawing line of interest
                hMillstone = plot(fig1Axes,longMillstone,latMillstone,gif_Millstone_Marker,'Color',gif_Millstone_Marker_Color,'MarkerSize',gif_Millstone_Marker_Size); %plots a point with a red big *
            else %projection requires converting to X/Y coords and then using them if plotting normally (mapping plot functions can use degc still - like geoshow)
                %Do the TEC plotting
                h = scatter(fig1Axes,pplong_Circ(k),pplat_Circ(k),gif_Scatter_Point_Size,sTEC(k),'filled');
                caxis(fig1Axes,[-gif_sTEC_cap, gif_sTEC_cap]);
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                colormap(fig1Axes,'jet') %heat map
                hBar = colorbar(fig1Axes); %shows the color bar 
                ...
                %Now drawing line of interest
                hMillstone = plot(fig1Axes,longMillstone_Circ,latMillstone_Circ,gif_Millstone_Marker,'Color',gif_Millstone_Marker_Color,'MarkerSize',gif_Millstone_Marker_Size); %plots a point with a red big *
            end
        else
            if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
        ...
        if( gif_MP4 == 1 )
            writeVideo(v,getframe(fig1)); %makes an MP4 file
        else
            %Otherwise use gif code
            im = frame2im(getframe(fig1));
            [imind,cm] = rgb2ind(im,256);
            if( FLG_firstPic == 1 )

              imwrite(imind,cm,gif_Name,'gif', 'Loopcount',inf,'DelayTime',gif_Delay);
              FLG_firstPic = 0; %resets that flag, done here

            else

              imwrite(imind,cm,gif_Name,'gif','WriteMode','append','DelayTime',gif_Delay);
...
elseif(gif_Type == 1) %stationary data point gif implementation
    ...
    for( i = 1:length(timeUnique) )
        ...
        %----------------Corral the data to the right place---------------- 
        k = find( time == timeUnique(i)); %gets during a time period
        sTEC_portion = sTEC(k); %get the sTEC at the time required
        if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
            pplat_portion = pplat(k); %get the pplat at the time required
            pplong_portion = pplong(k); %get the pplong at the time required
        else %projection requires converting to X/Y coords and then using them if plotting normally (mapping plot functions can use degc still - like geoshow)
            pplat_portion = pplat_Circ(k); %get the pplat at the time required
            pplong_portion = pplong_Circ(k); %get the pplong at the time required
        end
        
        gif_Grid = NaN*ones(gif_Grid_Long_Spaces+1,gif_Grid_Lat_Spaces+1); %preallocate sTEC grid
        
        if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob - linear delta works
            parfor(j = 1:gif_Grid_Lat_Spaces) %gets the sTEC in the right area
                for(l = 1:gif_Grid_Long_Spaces)
    ...
    %                     (pplong_portion >= (gif_Grid_Long(l) - gif_Grid_Long_Delta/2));
                    ...
                    %tedius checks have shown pcolor's boxes follow above math

                    if( sum(jk) ~= 0 )
                        temp = sTEC_portion(jk); %choose just the data desired
                        tempMean = mean( temp ); %get the mean
                        tempVar = var( temp ); %get variance
                        ...
                        if( dataReject > dataRejectMax ) %if this occured leave the data, it is too sparse or too varied to deal with effectively
                            ...
                            temp = temp( ((tempMean+dataReject*tempVar) > temp) & ((tempMean-dataReject*tempVar) < temp) ); %delete some extraneous data
                            %this is normal operation
                            gif_Grid(l,j) = mean(temp); %TECU, average data at the long/lat desired at the desired time
                        end
                    end
            ...
            parfor(j = 1:gif_Grid_Lat_Spaces) %gets the sTEC in the right area
                for(l = 1:gif_Grid_Long_Spaces)
                    jk = inpolygon(pplong_portion,pplat_portion,[gif_Grid_Long_Circ(j,l),gif_Grid_Long_Circ(j,l+1),gif_Grid_Long_Circ(j+1,l+1),gif_Grid_Long_Circ(j+1,l),gif_Grid_Long_Circ(j,l)],[gif_Grid_Lat_Circ(j,l),gif_Grid_Lat_Circ(j,l+1),gif_Grid_Lat_Circ(j+1,l+1),gif_Grid_Lat_Circ(j+1,l),gif_Grid_Lat_Circ(j,l)]);
                    %check using inpolygon call instead b/c of the shape n such
%                     jk = (pplat_portion <= (gif_Grid_Lat_Circ(j+1,l+1))) &...
%                         (pplat_portion >= (gif_Grid_Lat_Circ(j,l))) &...
%                         (pplong_portion <= (gif_Grid_Long_Circ(j+1,l+1))) &...
%                         (pplong_portion >= (gif_Grid_Long_Circ(j,l)));
                    %tedius checks have shown pcolor's boxes follow above math
                    %Uses a mesh grid of all points b/c of all 
        ...
        %---------------This was for interpolation, ditching it------------
%         [gif_Grid_Coords_X,gif_Grid_Coords_Y] = meshgrid(gif_Grid_Long,gif_Grid_Lat); %make two matrixes that have all the corresponding points
%         %they are appended as _NaN to be reused later on #memsave (will
%         %cause confusion though, becareful)
%         gif_Grid_isNaN = isnan(gif_Grid); %find all of the NaNs in gif_Grid coming out of it
% %         gif_Grid_NoNaN = ~isnan(gif_Grid); %find all of the NaNs in gif_Grid coming out of it
% 
%         gif_Grid_Coords_X_NoNaN = reshape(gif_Grid_Coords_X,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the X points, no NaN points
%         gif_Grid_Coords_X_NoNaN(reshape(gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
%         gif_Grid_Coords_Y_NoNaN = reshape(gif_Grid_Coords_Y,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the Y points, no NaN points
%         gif_Grid_Coords_Y_NoNaN(reshape(gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
%         gif_Grid_NoNaN = reshape(gif_Grid,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the TEC data, no NaN points
%         gif_Grid_NoNaN(reshape(gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
% %         gif_Grid_NoNaN = reshape(gif_Grid_NoNaN,length(gif_Grid_Coords_X_NoNaN),length(gif_Grid_Coords_Y_NoNaN));
% 
%         gif_Grid_Coords_X_NaN = reshape(gif_Grid_Coords_X,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the X points, no NaN points
%         gif_Grid_Coords_X_NaN(reshape(~gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
%         gif_Grid_Coords_Y_NaN = reshape(gif_Grid_Coords_Y,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the Y points, no NaN points
%         gif_Grid_Coords_Y_NaN(reshape(~gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
% %         gif_Grid_NaN = reshape(gif_Grid,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1); %vectorize the TEC data, no NaN points
% %         gif_Grid_NaN(reshape(~gif_Grid_isNaN,gif_Grid_Long_Spaces*gif_Grid_Lat_Spaces,1)) = []; %remove the NaN data, also vectorizes due to dark magic
% 
%         F = scatteredInterpolant(gif_Grid_Coords_X_NoNaN,gif_Grid_Coords_Y_NoNaN,gif_Grid_NoNaN,'linear','none'); %get a scattered 2D function
%         gif_Grid_NaN = F(gif_Grid_Coords_X_NaN,gif_Grid_Coords_Y_NaN); %get the NaN data estimated from the scattered 2D function
% 
%         gif_Grid(gif_Grid_isNaN) = gif_Grid_NaN; % apply the data into the system

%         gif_Grid(gif_Grid_isNaN) = interp2(gif_Grid_Coords_X,gif_Grid_Coords_Y,gif_Grid,gif_Grid_Coords_X_NaN,gif_Grid_Coords_Y_NaN,'cubic');
        %I can't get interp2 to work because it needs a gridded thing -
        %does not help if I apply it post-estimation (makes sense I guess)

    %     if( (isempty(k) == 0) && (length(k) > 5) ) %If stuff exsits or is more than 5 data points, print it up
        ...
        %-------------------Starting the Plotting--------------------------
        %Prep plot space
        %image prints upside down for some reason so requires flipping the axes twi
        %geoshow has white bands that I can't get rid of
        %pcolor has grids that need to be removed - but apparently cause issues in
        %pdf printing of image so image is used
%             imagesc(longRange,latRange,zeros(size(longRange,1),size(latRange,1)) ) % preps plot space, will be overwritten
%             set(gca,'Ydir','reverse') %fixes the axis orientation for this
%             caxis([0, dTECcap]);
%             colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
%             hold on; %hold holds for next plot to combine together
%             set(gca,'Ydir','Normal') %lets everything else print onto the orientation normally

        %Two options: image for non-interpolation (faster) or pcolor for
        %interpolation
    %     image(longRange,latRange,vtecDeltPlotter,'CDataMapping','scaled') % pseudocolor plot "stretched" to the grid
    %     image(longRange,latRange,vtecDeltPlotter); %psuedocolor plot "stretched" to the grid
    %     imagesc(longRange,latRange,vtecDeltPlotter) % pseudocolor plot "stretched" to the grid
        %OR what is used below
        
        %-------------------Starting the Plotting--------------------------
        if( i == 1 ) %first run preps axes, color bars, etc.
            if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
                %Do the TEC plotting
                h = pcolor(fig1Axes,gif_Grid_Long,gif_Grid_Lat,gif_Grid');% pseudocolor plot "stretched" to the grid - note gif_Grid is 1 larger than req b/c last row/column ignored am I right
                set(h, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
    %             shading interp; %stuff for interpolation
    %             lighting phong; %stuff for interpolation
                %NOTE: Above does not get along with geoshow polygon stuff
                caxis(fig1Axes,[-gif_sTEC_cap, gif_sTEC_cap]);
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                colormap(fig1Axes,'jet') %heat map
                hBar = colorbar(fig1Axes); %shows the color bar
                ...
                %Do the TEC plotting
                h = pcolor(fig1Axes,gif_Grid_Long_Circ,gif_Grid_Lat_Circ,gif_Grid');% pseudocolor plot "stretched" to the grid
                set(h, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
    %             shading interp; %stuff for interpolation
    %             lighting phong; %stuff for interpolation
                %NOTE: Above does not get along with geoshow polygon stuff
        ...
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
            ...
            if( gif_plotProjection == 0 )
                %Just plotting now - prep done, hopefully speeds it!
                %Do the TEC plotting
                h = pcolor(fig1Axes,gif_Grid_Long,gif_Grid_Lat,gif_Grid');% pseudocolor plot "stretched" to the grid
                set(h, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
    %             shading interp; %stuff for interpolation
    %             lighting phong; %stuff for interpolation
                %NOTE: Above does not get along with geoshow polygon stuff
...
elseif(gif_Type == 2) %stationary data points + Zenith ISR data at same time
    ...
    gif_Zenith_time_start = find( min(abs(min(Zenith_time) - timeUnique)) == abs(min(Zenith_time) - timeUnique) ); %match Zenith start time
    gif_Zenith_time_end = find( min(abs(max(Zenith_time) - timeUnique)) == abs(max(Zenith_time) - timeUnique) ); %match Zenith end time

    %-------------------------Start Making Pictures------------------------
    for( i = gif_Zenith_time_start:gif_Zenith_time_end )
...
%                         (pplong_portion >= (gif_Grid_Long_Circ(j,l)));
        ...
        %Interpolation disabled b/c obfuscates actual TEC action
%         [gif_Grid_Coords_X,gif_Grid_Coords_Y] = meshgrid(gif_Grid_Long,gif_Grid_Lat); %make two matrixes that have all the corresponding points
%         %they are appended as _NaN to be reused later on #memsave (will
%         %cause confusion though, becareful)
%         gif_Grid_isNaN = isnan(gif_Grid); %find all of the NaNs in gif_Grid coming out of it
...
%         gif_Grid_NaN = F(gif_Grid_Coords_X_NaN,gif_Grid_Coords_Y_NaN); %get the NaN data estimated from the scattered 2D function
% 
%         if(isempty(gif_Grid_NaN) ~= 1)
%             gif_Grid(gif_Grid_isNaN) = gif_Grid_NaN; % apply the data into the system
%         else
%             %seems to occur if all data is NaN, call it a day do nothing
%         end
        ...
        if( i == gif_Zenith_time_start ) %first run preps axes, color bars, etc.
            if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
                %Do the TEC plotting
                subplot(2,1,1,fig1Axes); %upper bit, transforms prev plot
                h = pcolor(fig1Axes,gif_Grid_Long,gif_Grid_Lat,gif_Grid');% pseudocolor plot "stretched" to the grid - note gif_Grid is 1 larger than req b/c last row/column ignored am I right
                set(h, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
    %             shading interp; %stuff for interpolation
    %             lighting phong; %stuff for interpolation
        ...
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                ...
                ylabel(hBar, 'delta-sTEC (TECU)','fontweight',font_Weight, 'FontSize',font_Size); %labels color bar
                set(fig1Axes,'xtick',min(plotLongRange):plotLongRange_autoTick:max(plotLongRange)); %set standardized limit
                set(fig1Axes,'ytick',min(plotLatRange):plotLatRange_autoTick_Crunched:max(plotLatRange)); %set standardized limit
                ...
                %Do the ISR Zenith plotting
                %Prep plot space
                ax1 = subplot(2,1,2); %lower bit
                hISR = pcolor(ax1, (Zenith_time - dateRange_zeroHr(2)).*24 ,Zenith_height,Zenith_SNR_bp');
                set(hISR, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
                shading interp;
                lighting phong; %these make it look good
                caxis(ax1,[-0.1 0.1]); %limit SNR
                colormap(ax1,'gray'); %gray scale shows it nicely
                hISRBar = colorbar(ax1); %shows the color bar
                ylabel(hISRBar, 'SNR (Unitless)','fontweight',font_Weight, 'FontSize',font_Size); %labels color bar
                ylim([90 700]); %km, altitude limit
                % ylim([90 500]);
                % xlim([0 70]);
                xlabel(ax1,['Time in UT - 0 Hr at 0 UT ',dateRange_zeroHr_YrMonDay_string]);
                ylabel(ax1,'Height (km)');
                title(ax1,'Zenith SNR 2 Hr Highpass Filtered');
                set(gca,'fontweight',font_Weight, 'FontSize',font_Size);
                set(ax1,'xtick',plotTime_ISR_autoTick);
                gk = find( min(abs(Zenith_time - timeUnique(i) )) == abs(Zenith_time - timeUnique(i) ) ); %gets closest Zenith_time to the timeUnique value given
                hISRLine = line(ax1,[ (Zenith_time(gk)-dateRange_zeroHr(2)).*24 , (Zenith_time(gk)-dateRange_zeroHr(2)).*24 ],[min(Zenith_height),max(Zenith_height)],'Color','b','LineWidth',1);
                %add on a vertical line that shows where we are at in time
...
elseif(gif_Type == 3) %stationary data points + MISA ISR data at same time
    %------------------------Initialize Video------------------------------
    if( gif_MP4 == 1 ) %open here to support last-minute changes to FPS, etc
        open(v); %open video
    end
    ...
    %-----------------------Clip Time to ISR Zenith------------------------
    gif_MISA_time_start = find( min(abs(min(MISA_time) - timeUnique)) == abs(min(MISA_time) - timeUnique) ); %match start time
    gif_MISA_time_end = find( min(abs(max(MISA_time) - timeUnique)) == abs(max(MISA_time) - timeUnique) ); %match end time

    for( i = gif_MISA_time_start:gif_MISA_time_end )
...
%                         (pplong_portion >= (gif_Grid_Long_Circ(j,l)));
        ...
        %Interp disabled b/c obfuscates TEC
%         [gif_Grid_Coords_X,gif_Grid_Coords_Y] = meshgrid(gif_Grid_Long,gif_Grid_Lat); %make two matrixes that have all the corresponding points
%         %they are appended as _NaN to be reused later on #memsave (will
%         %cause confusion though, becareful)
%         gif_Grid_isNaN = isnan(gif_Grid); %find all of the NaNs in gif_Grid coming out of it
...
%         gif_Grid(gif_Grid_isNaN) = interp2(gif_Grid_Coords_X,gif_Grid_Coords_Y,gif_Grid,gif_Grid_Coords_X_NaN,gif_Grid_Coords_Y_NaN,'cubic');
    ...
    %             lighting phong; %stuff for interpolation
        ...
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                ...
                %Now drawing line of interest
                hMillstone = plot(fig1Axes,longMillstoneMISA,latMillstoneMISA,gif_Millstone_Marker,'Color',gif_Millstone_Marker_Color,'MarkerSize',gif_Millstone_Marker_Size); %plots a point with a red big *

                %Do the ISR MISA plotting
                %Prep plot space
                ax1 = subplot(2,1,2); %lower bit
                hISR = pcolor(ax1, (MISA_time - dateRange_zeroHr(2)).*24 ,MISA_height,MISA_SNR_bp');
                set(hISR, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
                shading interp;
                lighting phong; %these make it look good
                caxis(ax1,[-0.1 0.1]); %limit SNR
                ...
                xlabel(ax1,['Time in UT - 0 Hr at 0 UT ',dateRange_zeroHr_YrMonDay_string]);
                ylabel(ax1,'Height (km)');
                title(ax1,'MISA SNR 2 Hr Highpass Filtered');
                set(gca,'fontweight',font_Weight, 'FontSize',font_Size);
                set(ax1,'xtick',plotTime_ISR_autoTick);
                gk = find( min(abs(MISA_time - timeUnique(i) )) == abs(MISA_time - timeUnique(i) ) ); %gets closest MISA_time to the timeUnique value given
                hISRLine = line(ax1,[ (MISA_time(gk)-dateRange_zeroHr(2)).*24 , (MISA_time(gk)-dateRange_zeroHr(2)).*24 ],[min(MISA_height),max(MISA_height)],'Color','b','LineWidth',1);
                %add on a vertical line that shows where we are at in time
    ...
    %             lighting phong; %stuff for interpolation
        ...
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                ...
                %Now drawing line of interest
                hMillstone = plot(fig1Axes,longMillstoneMISA_Circ,latMillstoneMISA_Circ,gif_Millstone_Marker,'Color',gif_Millstone_Marker_Color,'MarkerSize',gif_Millstone_Marker_Size); %plots a point with a red big *
...
elseif(gif_Type == 4)
    ...
    gif_TimeAvg = AMPERE_timeStep*60; %min, force AVG to match the AMPERE time step (only for reporting at this point)
    
    timeUnique_TimeAvg = AMPERE_time_range; %hr, set up the time steps to be exactly what the AMPERE data time steps are
    if( timeUnique_TimeAvg(end) < (max(timeUnique)-dateRange_zeroHr(2))*24 )
        timeUnique_TimeAvg(end+1) = (max(timeUnique)-dateRange_zeroHr(2))*24; %hr, force last time range to be added if it didn't fit in well
    end
    %makes coding easier
    
    %Stuff for repeating a frame - needed if want a constant 30 FPS
    %This needs to go in gif_Type == 2 area
    gif_DesiredTotalTime = (length(timeUnique_TimeAvg)-1)/gif_DesiredFPS; %s, total time from desired FPS
    gif_DesiredReqFrameNum = gif_DesiredTotalTime*30; %frames, number of frames needed to go for the time wanted
    gif_RepeatFrame = gif_DesiredReqFrameNum/(length(timeUnique_TimeAvg)-1); %number of frames to repeat every time (exact w/ decimal)
    if( gif_RepeatFrame > 0.5 )
        gif_RepeatFrame = round(gif_RepeatFrame); %round it
        ...
        if( ( (length(timeUnique_TimeAvg)-1)/30 > gif_DesiredMaxRunTime ) && ( gif_FLGdisableFPSShift == 0 ) ) %try to keep run time down
            v.FrameRate = 60; %set the FPS to 60
        end
    end
    ...
    %-------------------------Start Making Pictures------------------------
    for( i = 1:(length(timeUnique_TimeAvg)-1) )
        
        %------------Corral the AMPERE data to the right place-------------
        k = AMPERE_time == timeUnique_TimeAvg(i); %get where the time point is
        AMPERE_jouleHeating_portion = AMPERE_data(k,AMPERE_jouleHeating_pos); %ergs/(cm^2*sec), get the Joule Heating for the current time step
        if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
            AMPERE_lat_portion = AMPERE_lat(k,1); %degc, corresponding lat values
            AMPERE_long_portion = AMPERE_long(k,1); %degc, corresponding long values
        else %projection requires converting to X/Y coords and then using them if plotting normally (mapping plot functions can use degc still - like geoshow)
            AMPERE_lat_portion = AMPERE_lat_Circ(k,1); %degc, corresponding lat values
            AMPERE_long_portion = AMPERE_long_Circ(k,1); %degc, corresponding long values
        end
        
        %----------------Corral the data to the right place---------------- 
        k = ( time >= (timeUnique_TimeAvg(i)/24+dateRange_zeroHr(2)) ) & ( time < (timeUnique_TimeAvg(i+1)/24+dateRange_zeroHr(2)) ); %get where time points fall within the averaging boundaries
        sTEC_portion = sTEC(k); %get the sTEC at the time required
        if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
            pplat_portion = pplat(k); %get the pplat at the time required
            pplong_portion = pplong(k); %get the pplong at the time required
...
%                         (pplong_portion >= (gif_Grid_Long_Circ(j,l)));
        ...
        %----------------------------Tack on Title-------------------------
        string_Title = ['TEC Global Plot | Time Avg''d to Every ',num2str(gif_TimeAvg),' Min | Time = ',num2str(round(timeUnique_TimeAvg(i+1),2),'%10.2f'),' hr, 0 UT on ',dateRange_zeroHr_YrMonDay_string]; %create title string
        if( FLG_geomagneticCoords == 1)
            string_Title = [string_Title,' [Mag Coord Aligned]']; %tack on mag coordinate warning
        end
        title(fig1Axes,string_Title);
    ...
    %             lighting phong; %stuff for interpolation
        ...
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                ...
                set(fig1Axes,'xtick',min(plotLongRange):plotLongRange_autoTick:max(plotLongRange)); %set standardized limit
                set(fig1Axes,'ytick',min(plotLatRange):plotLatRange_autoTick:max(plotLatRange)); %set standardized limit
                defaultPosition = get(fig1Axes,'Position'); %get latest fig position (colorbar addition moves it slightly)
                
                %PLOT THE AMPERE
                ax2 = axes; %create independent axes for the two plots (TEC and AMPERE)
                linkaxes([fig1Axes,ax2]); %link em
                xlim(ax2,[min(plotLongRange), max(plotLongRange)]); %force axis to be this
                ylim(ax2,[min(plotLatRange), max(plotLatRange)]); %force axis to be this
                set(ax2,'Position',defaultPosition); %force this to be at same position (won't be b/c no colorbar)
                hold on; %keep from deleting old axes
                h2 = scatter(ax2,AMPERE_long_portion,AMPERE_lat_portion,gif_Scatter_Point_Size_AMPERE,AMPERE_jouleHeating_portion,'filled');
                ax2.Visible = 'off'; %hide the 2nd axes set
                ax2.XTick = []; %hide the 2nd axes set
                ax2.YTick = []; %hide the 2nd axes set
                caxis(ax2,[min(gif_AMPERE_jouleHeating_cap), max(gif_AMPERE_jouleHeating_cap)]);
                %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                myMap = [[linspace(1,0.492063492063492,32)',linspace(1,0.507936507936508,32)',linspace(1,1,32)'];[linspace(0.492063492063492,1,32)',linspace(0.507936507936508,0,32)',linspace(1,1,32)']]; %white to purpleblue to pink (based off of 'cool')
    %             myMap = [linspace(1,1,64)',linspace(1,0,64)',linspace(1,1,64)']; %white to pink based off of 'cool'
                colormap(ax2,myMap) %heat map
                h2Bar = colorbar(ax2,'Position',[0.0707 0.1104 0.0140 0.8146]); %shows the color bar
        %         h2Bar = colorbar(ax2,'Location','westoutside');
                %manual position is req to place well
                ylabel(h2Bar, 'AMPERE Joule Heating (erg/cm^{2}\bulletsec)','fontweight','bold','FontSize',18); %labels color bar
        %         ax2.Position = fig1Axes_pos + [.012 , .0130 , -0.023 , -0.023]; %manually-found points to align to circular grid size
                ...
                %Do the TEC plotting
                ax1 = axes; %create independent axes for the two plots (TEC and AMPERE)
                linkaxes([fig1Axes,ax1]); %link em
                xlim(ax1,[min(plotLongRange), max(plotLongRange)]); %force axis to be this
                ylim(ax1,[min(plotLatRange), max(plotLatRange)]); %force axis to be this
                set(ax1,'Position',defaultPosition); %force this to be at same position (won't be b/c no colorbar)
                h = pcolor(ax1,gif_Grid_Long_Circ,gif_Grid_Lat_Circ,gif_Grid');% pseudocolor plot "stretched" to the grid
%                 h = surf(fig1Axes,gif_Grid_Long_Circ,gif_Grid_Lat_Circ,1000*ones(size(gif_Grid_Long_Circ)),gif_Grid'); %fix for stupid stuff like ax2 always on top b/c Z is dumb and all 0 so this isn't all 0
%                 view([0 90]); %makes it pcolor-like
                set(h, 'EdgeColor', 'none'); %remove grid lines (which make it look hilarious)
    %             shading interp; %stuff for interpolation
    %             lighting phong; %stuff for interpolation
                %NOTE: Above does not get along with geoshow polygon stuff
                ax1.Visible = 'off'; %hide the 2nd axes set
                ax1.XTick = []; %hide the 2nd axes set
                ax1.YTick = []; %hide the 2nd axes set
                caxis(ax1,[-gif_sTEC_cap, gif_sTEC_cap]);
        %         colormap(flipud(colormap('hot'))) %heat map where 0 (low) is white
                colormap(ax1,'jet') %heat map
                hBar = colorbar(ax1); %shows the color bar
                ylabel(hBar, 'delta-sTEC (TECU)','fontweight',font_Weight, 'FontSize',font_Size); %labels color bar
                defaultPosition = get(fig1Axes,'Position'); %get latest fig position (colorbar addition moves it slightly)
    ...
    %             lighting phong; %stuff for interpolation
        ...
        if( gif_MP4 == 1 )
            %use MP4 code (ez)
            for( ii = 1:gif_RepeatFrame) %repeats frame to reach an effective FPS (locked to 30 FPS on output)
                writeVideo(v,getframe(fig1)); %makes an MP4 file
            end
        else
            %Otherwise use gif code
...
elseif(gif_Type == 5)
    ...
    for( i = 1:length(timeUnique) )
        ...
        %------------Corral the AMPERE data to the right place-------------
        k = find((AMPERE_time_range < (timeUnique(i)-dateRange_zeroHr(2))*24) | (AMPERE_time_range == (timeUnique(i)-dateRange_zeroHr(2))*24)); %get where the time point is on the range
        k = AMPERE_time == AMPERE_time_range(k(end)); %connect the last range (which is the one we want) to the AMPERE data
        AMPERE_jouleHeating_portion = AMPERE_data(k,AMPERE_jouleHeating_pos); %ergs/(cm^2*sec), get the Joule Heating for the current time step
        if( gif_plotProjection == 0 ) %no projection means can plot in lat/long degc no prob
            AMPERE_lat_portion = AMPERE_lat(k,1); %degc, corresponding lat values
            AMPERE_long_portion = AMPERE_long(k,1); %degc, corresponding long values
...
%                         (pplong_portion >= (gif_Grid_Long_Circ(j,l)));
        ...
        %----------------------------Tack on Title-------------------------
        string_Title = ['TEC Global Plot | AMPERE Time Avg''d to Every ',num2str(round(AMPERE_timeStep*60)),' Min | Time = ',num2str(round((timeUnique(i)-dateRange_zeroHr(2))*24,2),'%10.2f'),' hr, 0 UT on ',dateRange_zeroHr_YrMonDay_string]; %create title string
        if( FLG_geomagneticCoords == 1)
            string_Title = [string_Title,' [Mag Coord Aligned]']; %tack on mag coordinate warning
        end
        title(fig1Axes,string_Title);
    ...
    %             myMap = [linspace(1,1,64)',linspace(1,0,64)',linspace(1,1,64)']; %white to pink based off of 'cool'
        ...
        %         ax2.Position = fig1Axes_pos + [.012 , .0130 , -0.023 , -0.023]; %manually-found points to align to circular grid size
                ...
                hLgd = 0; %plot handle, need to init it to check on it initally I know right
                hLgd_FLG_both = 0 ; %plotting flag, shows when legend is active
                hLgd_FLG_day = 0; %plotting flag, shows when legend is active
                hLgd_FLG_nite = 0; %plotting flag, shows when legend is active
                dayNite_tempDelta = abs( (timeUnique(i)-dateRange_zeroHr(2))*24 - dayNite_sunrise(:,:) ); %get the temp delta
                dayNite_temp = min(dayNite_tempDelta,[],2 ) == dayNite_tempDelta;
                dayNite_temp( dayNite_tempDelta > 2/60 ) = 0; %clear out matches that have too much deviation (2 min chosen for now - 5/10 caused an extra hit too often)
                hDayNite_day = plot(fig1Axes,dayNite_Grid_Long(dayNite_temp),dayNite_Grid_Lat(dayNite_temp),'Color','g','LineWidth',1.75); %plots a line to show the sunrise time
                
                dayNite_tempDelta = abs( (timeUnique(i)-dateRange_zeroHr(2))*24 - dayNite_sunset(:,:) ); %get the temp delta
                dayNite_temp = min(dayNite_tempDelta,[],2 ) == dayNite_tempDelta;
                dayNite_temp( dayNite_tempDelta > 2/60 ) = 0; %clear out matches that have too much deviation (2 min chosen for now - 5/10 caused an extra hit too often)
                hDayNite_nite = plot(fig1Axes,dayNite_Grid_Long(dayNite_temp),dayNite_Grid_Lat(dayNite_temp),'Color','b','LineWidth',1.75); %plots a line to show the sunrise time
...
%                 view([0 90]); %makes it pcolor-like
    ...
    %             lighting phong; %stuff for interpolation
        ...
        if( (isempty(hDayNite_day) == 0) && (isempty(hDayNite_nite) == 0) && (hLgd_FLG_both == 0) ) %both there put em both magic
            if( isgraphics(hLgd,'legend') ) %delete old one if we need to update it here
                delete(hLgd);
            end
            hLgd = legend(fig1Axes,[hDayNite_day, hDayNite_nite],{'Sunrise','Sunset'},'Location','southeast'); %put it up n out
            set(hLgd,'Box','off'); %turns off box
            hLgd_FLG_both = 1; %turn on flg that notes legend is there
            hLgd_FLG_day = 0; %turn off other legend flags
            hLgd_FLG_nite = 0; %turn off other legend flags
        elseif( (isempty(hDayNite_day) == 0) && (isempty(hDayNite_nite) == 1) && (hLgd_FLG_day == 0) ) %sunrise is visible
            if( isgraphics(hLgd,'legend') ) %delete old one if we need to update it here
                delete(hLgd);
            end
            hLgd = legend(fig1Axes,hDayNite_day,{'Sunrise'},'Location','southeast'); %put it up n out
            set(hLgd,'Box','off'); %turns off box
            hLgd_FLG_both = 0; %turn off other legend flags
            hLgd_FLG_day = 1; %turn on flg that notes legend is there
            hLgd_FLG_nite = 0; %turn off other legend flags
        elseif( (isempty(hDayNite_day) == 1) && (isempty(hDayNite_nite) == 0) && (hLgd_FLG_nite == 0)  ) %sunset is visible
            if( isgraphics(hLgd,'legend') ) %delete old one if we need to update it here
                delete(hLgd);
            end
            hLgd = legend(fig1Axes,hDayNite_nite,{'Sunset'},'Location','southeast'); %put it up n out
            set(hLgd,'Box','off'); %turns off box
            hLgd_FLG_both = 0; %turn off other legend flags
            hLgd_FLG_day = 0; %turn off other legend flags
            hLgd_FLG_nite = 1; %turn on flg that notes legend is there
        end
        ...
        delete(hOverlay);
        delete(hDayNite_day);
        delete(hDayNite_nite);
        if( (isempty(hDayNite_day) == 1) && (isempty(hDayNite_nite) == 1) && ((hLgd_FLG_both == 1) || (hLgd_FLG_day == 1) || (hLgd_FLG_nite == 1)) ) %only happens if both empty but legend still on
            delete(hLgd); %remove legend
            hLgd_FLG_both = 0; %turn off other legend flags
            hLgd_FLG_day = 0; %turn off other legend flags
            hLgd_FLG_nite = 0; %turn off other legend flags
        end
...
end %end of MOVIE MAKING IF STATEMENT

toc
close(v) %close the video file

cd(origDirectory); %return to the original directory

end %*********END OF FLG_enable_videoCreation*********